...
if __name__ == "__main__":
    ...
    data_list = [d for d in data_list if '.' not in d.strip().split()[0]]  # strip()去除首尾空格，split()分割字符串中间空格
    N = len(data_list)

    compounds, adjacencies, proteins, interactions = [], [], [], []
    model = Word2Vec.load("word2vec_30.model")
    for no, data in enumerate(data_list):
        print('/'.join(map(str, [no + 1, N])))  # map()函数会根据提供的函数对指定序列做映射
        smiles, sequence, interaction = data.strip().split(" ")  # 显示的是data数据每一条的smiles、seq、inter数值
