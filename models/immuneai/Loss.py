...
# --- Lazy RNN model loading ---
# clone and vocab are None until load_rnn_model() is called.
clone = None
vocab = None
_rnn_device = try_gpu()

_RNN_MODEL_PATH = None  # Injected by run_immuneai() or set_rnn_model_path()


def set_rnn_model_path(path: str, data_dir: str, data_file: str = 'student_train.log'):
    """Call this before using structure loss to enable the RNN component."""
    global clone, vocab, _RNN_MODEL_PATH
    _RNN_MODEL_PATH = path
    try:
        lines = read_sequence(data_dir, data_file)
        tokens = tokenize(lines)
        import numpy as _np
        tokens = [list(_) for _ in _np.array(tokens)[:, 1]]
        vocab = Vocab(tokens)

        num_hiddens, num_layers = 64, 1
        ffn_num_hiddens = linear1_num_hiddens = linear2_num_hiddens = 32
        linear3_num_hiddens = 8
        rnn_layer = nn.LSTM(len(vocab), num_hiddens, num_layers)
        clone = RNNModel(rnn_layer, len(vocab), ffn_num_hiddens,
                         linear1_num_hiddens, linear2_num_hiddens, linear3_num_hiddens,
                         dropout=0.5)
        clone.load_state_dict(torch.load(path, map_location=_rnn_device))
        clone.to(_rnn_device)
        clone.eval()
    except Exception as e:
        print(f"[ImmuneAI Warning] RNN model not loaded ({e}). structure_loss will be 0.")
        clone = None
        vocab = None
...
def IEDB_loss(iedb_freqmat, pep_seq):
    ...
    return sum(-np.log(i) for i in iedb_prob)


def pep_TCR_loss(prob_ref_freqmat, pep_seq, cdr_seq, TCRpos):
    if cdr_seq is None:
        pep_cdr_discrete = [
            0.4000000000000001, 3.3333333333333335, 1.6333333333333335, 2.0,
            0.14285714285714285, 1.2666666666666664, 0.8437500000000001,
            0.4129032258064516, 1.4, 1.5318181818181817, 0.4273584905660377,
            3.36, 1.5727272727272728, 1.3707317073170735, 1.4407407407407407,
            0.15384615384615385, 1.3511627906976744, 2.5333333333333337,
            2.032258064516129, 0.5537313432835821,
        ]
        ...
        loss = 0
        for n, i in enumerate(tcr_prob, start=1):
            if n in TCRpos:
                loss += -np.log(i) if i != 0 else -np.log(1e-20)
        return loss
    else:
        return 0.0  # explicit_pep_TCR_loss not implemented


def compute_loss(iedb_freqmat, prob_ref_freqmat, pep_seq, logfile, w1=10, w2=-0.1, w3=None, w4=None):
    global args, clone, vocab, _rnn_device

    if w4 is None:
        w4 = args.weight_iedb
    if w3 is None:
        w3 = args.weight_cdr if args.cdr_sequence is not None else args.weight_cdr_dis

    cdr_loss = pep_TCR_loss(
        prob_ref_freqmat, pep_seq=pep_seq,
        cdr_seq=args.cdr_sequence, TCRpos=args.TCR_loss_pos,
    )
    iedb_loss = IEDB_loss(iedb_freqmat, pep_seq)

    if clone is not None and vocab is not None:
        structure_loss = predict_single(pep_seq, clone, vocab, _rnn_device).item()
    else:
        structure_loss = 0.0

    print("", file=logfile)
    print("LOSS RESULTS of the {}:".format(list(pep_seq)), file=logfile)
    if args.cdr_sequence is None:
        print("****cdr loss --> {} given no cdr".format(w3 * cdr_loss), file=logfile)
    else:
        print("****cdr loss --> {} with cdr sequence as {}".format(
            w3 * cdr_loss, args.cdr_sequence), file=logfile)
    print("****iedb loss --> {}".format(w4 * iedb_loss), file=logfile)
    print("****structure loss --> {}".format(structure_loss), file=logfile)

    return w3 * cdr_loss + w4 * iedb_loss + structure_loss
